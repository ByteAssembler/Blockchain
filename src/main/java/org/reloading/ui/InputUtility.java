...
public class InputUtility {
    public static Optional<Transaction> createTransactionDialog() {
        if (Accounts.getAccount().size() < 2) {
            JOptionPane.showMessageDialog(null, "At least two accounts are required to create a transaction. Please create an account first.", "Error", JOptionPane.ERROR_MESSAGE);
        ...
        if (sender.getPersonName().equalsIgnoreCase(receiver.getPersonName())) {
            JOptionPane.showMessageDialog(null, "Sender and receiver cannot be the same. Please enter different names.", "Error", JOptionPane.ERROR_MESSAGE);
        ...
        if (amount <= 0) {
            JOptionPane.showMessageDialog(null, "Amount must be greater than 0. Please enter a valid amount.", "Error", JOptionPane.ERROR_MESSAGE);
            return createTransactionDialog();
        }
    ...
    public static Account getPersonDialog(Person.Type personType) {
        if (Accounts.getAccount().isEmpty()) {
            JOptionPane.showMessageDialog(null, "No accounts exist. Please create an account first.", "Error", JOptionPane.ERROR_MESSAGE);
            return null;
        }

        String name = JOptionPane.showInputDialog(null, "Enter " + personType + " name:", "Create " + personType, JOptionPane.PLAIN_MESSAGE);
        ...
        try {
            ...
            if (accountOption.isEmpty()) {
                JOptionPane.showMessageDialog(null, "Account with name " + name + " does not exist. Please enter a valid name.", "Error", JOptionPane.ERROR_MESSAGE);
                return getPersonDialog(personType);
            }
